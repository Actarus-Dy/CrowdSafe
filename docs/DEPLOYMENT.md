# CrowdSafe -- Deployment Guide

Deployment, configuration, and production operations for the CrowdSafe (C-01) API server.
...
```bash
uvicorn crowdsafe.api.app:app --host 0.0.0.0 --port 8000 --reload --reload-dir crowdsafe
```
...
```bash
docker build -t crowdsafe .
```

### Build -- with GPU support

```bash
docker build --build-arg INSTALL_GPU=1 -t crowdsafe:gpu .
```
...
```bash
docker run -p 8000:8000 crowdsafe
```

With GPU:

```bash
docker run --gpus all -p 8000:8000 crowdsafe:gpu
```
...
This starts the API server with `--reload` and a read-only bind mount of the `crowdsafe/` source directory, so code changes take effect immediately without rebuilding the image.
...
uvicorn crowdsafe.api.app:app --host 0.0.0.0 --port 8000
```
...
- Runs `ruff check` and `ruff format --check` against `crowdsafe/` and `tests/`.
...
- Builds the Docker image (`crowdsafe:ci`).
...
Returns HTTP 200 with `{"status": "ready", "n_pedestrians": <N>}` if a simulation is loaded. Returns HTTP 503 if no simulation exists. Use this for Kubernetes `readinessProbe` to avoid routing traffic before the simulation is initialized.
...
|-----------------------------------|---------|------------------------------------------|
| `crowdsafe_uptime_seconds`      | gauge   | Server uptime in seconds                 |
| `crowdsafe_step_count`          | counter | Total simulation steps completed         |
| `crowdsafe_n_pedestrians`          | gauge   | Current number of pedestrians               |
| `crowdsafe_mean_speed_kmh`      | gauge   | Mean pedestrian speed in km/h               |
| `crowdsafe_rate_limit_keys`     | gauge   | Active rate limiter keys (connection count proxy) |
...
CrowdSafe **must** run with a single uvicorn worker (`--workers 1`). The `CrowdSimulation` object holds mutable NumPy arrays and is not fork-safe. Multiple workers would create independent simulation copies with divergent state. The Dockerfile enforces this by default.

### Memory limits

- A 5x5 grid with 1,000 pedestrians uses approximately 200 MB.
- The Docker Compose production profile sets a 2 GB memory limit.
- For large simulations (10,000+ pedestrians), increase the limit accordingly:
...
CrowdSafe includes a GPU-accelerated O(N^2) force engine (`ForceEngineGPU`) that uses CuPy for CUDA computation. For the Janus signed-mass model, naive O(N^2) on GPU outperforms CPU Barnes-Hut O(N log N) for moderate N.

### Install CuPy

```bash
pip install "crowdsafe[gpu]"
# or directly:
...
`CrowdSimulation` auto-detects GPU availability at startup:
...
- You can force CPU mode by passing `use_gpu=False` to `CrowdSimulation`.

### `max_n` threshold

`ForceEngineGPU` has a `max_n` parameter (default: 10,000) that controls the upper bound for GPU computation. When the number of pedestrians exceeds `max_n`, the engine falls back to CPU naive computation to avoid GPU out-of-memory errors.

- 10,000 pedestrians requires approximately 1.5 GB of VRAM for the N x N distance and force matrices.
