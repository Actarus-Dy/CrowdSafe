# CrowdSafe -- Physics Reference

> **Project:** CrowdSafe (C-01) -- Actarus-Dy Software Catalogue
...
CrowdSafe maps this cosmological framework onto vehicular traffic:

- **Slow pedestrians** (speed below segment average) receive **positive mass**.
  They act as gravitational attractors, forming congestion seeds -- other
  pedestrians are pulled toward them, nucleating jams.

- **Fast pedestrians** (speed above segment average) receive **negative mass**.
  They act as repulsors, pushing nearby pedestrians apart and creating fluid,
  free-flowing zones.
...
- `crowdsafe/core/mass_assigner.py` -- mass computation
- `crowdsafe/core/force_engine.py` -- gravitational force computation
- `crowdsafe/core/simulation.py` -- full pipeline
...
The gravitational social force exerted on pedestrian *i* by pedestrian *j* is:
...
- `m_i`, `m_j` -- signed gravitational masses of pedestrians i and j
- `(x_j - x_i, y_j - y_i)` -- displacement vector pointing from i toward j

### Softened distance

To prevent the force from diverging when two pedestrians occupy nearly the same
...
| `v_mean`     | Population mean speed across all pedestrians    | m/s      |
...
After mass computation, pedestrians are classified for monitoring:
...
Pure gravity alone cannot generate a fundamental diagram. When all pedestrians
...
Gravity handles inter-pedestrian interactions; drag provides the baseline
...
| `\|v_i\|`     | Current speed magnitude of pedestrian i      | m/s    |
| `direction_i`  | Unit vector along pedestrian velocity        | --     |
...
The total acceleration on pedestrian *i* combines gravity and drag:
...
pedestrians (which would otherwise produce extreme accelerations from small
forces).

### Implementation

```python
# simulation.py, CrowdSimulation._compute_accelerations()
...
| Speed limit  | `v_max`    | 36.0    | m/s     | Maximum pedestrian speed (~130 km/h)      |
...
1. **Positive tree**: contains only pedestrians with `m_i >= 0`
2. **Negative tree**: contains only pedestrians with `m_i < 0`
...
CrowdSafe uses the **kick-drift-kick (KDK)** variant of the leapfrog
...
1. For each density in `[10, 30, 50, 70, 90, 110, 130]` veh/km:
   - Place pedestrians uniformly on a 2 km highway segment.
   - Initialize **all pedestrians at free-flow speed** (`v_free = 33.33 m/s`),
...
term. Gravity provides perturbations and inter-pedestrian coupling but does not
...
explicitly programmed. CrowdSafe measures emergence by comparing
**gravity-on** (`G_s=5.0`) vs. **gravity-off** (`G_s=0`, drag only) scenarios.

### Test scenario

- 100-200 pedestrians on a 2 km highway at uniform 25 m/s.
- One slow pedestrian injected at the midpoint (5 m/s).
...
The gravitational well created by the slow pedestrian (positive mass, attractor)
propagates a deceleration wave upstream. Pedestrians approaching from behind
decelerate before reaching the slow pedestrian -- a chain reaction mediated by
gravity.

```
upstream_decel = initial_speed - mean_final_speed(upstream_pedestrians)
```
...
The Gini coefficient measures clustering inequality in pedestrian spacings.
A Gini of 0 means perfectly uniform spacing; a Gini approaching 1 means
extreme clustering (jam). Gravity increases the Gini over time as pedestrians
cluster behind the slow pedestrian.

```
Gini = (2 * sum(i * x_i) - (n+1) * sum(x_i)) / (n * sum(x_i))
```

where `x_i` are sorted pedestrian spacings.

#### Shock wave speed

The furthest upstream pedestrian that decelerated by more than 0.5 m/s defines
...
pedestrian's own speed and the local density field. It does **not** reference
the speed or position of any leading pedestrian. It represents the macroscopic
tendency for pedestrians to adopt a density-appropriate speed (engine thrust
vs. aerodynamic drag analogy).

Inter-pedestrian interactions are handled exclusively by gravity. This separation
is a deliberate design choice: gravity provides the emergent, non-local
coupling that distinguishes CrowdSafe from car-following models like
...
fundamental diagram. When all pedestrians travel at the mean speed:
...
gravitational attraction on approaching pedestrians, simulating deceleration) but
...
### 10.6 PedestrianAgent is passive

In the Mesa ABM layer, `PedestrianAgent` does not compute its own physics. All
forces, accelerations, and position updates are computed centrally in
`CrowdSimulation` and pushed to agents. This prevents inconsistencies between
...
- Janus Civil C-01 CrowdSafe Technical Plan, Sections 1.2, 1.3, 2.1, 4.2
...
*Document generated 2026-03-23. Source of truth: `crowdsafe/core/` modules.*
