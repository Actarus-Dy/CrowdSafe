...
CrowdSafe exposes a REST API and a WebSocket endpoint for real-time gravitational crowd simulation based on the Janus Cosmological Model.
...
> **Limitation:** The `/metrics` endpoint exposes operational data (uptime, pedestrian count, mean speed). Restrict access to this endpoint to internal networks only.
...
```text
# HELP crowdsafe_uptime_seconds Server uptime in seconds
# TYPE crowdsafe_uptime_seconds gauge
crowdsafe_uptime_seconds 342.7
# HELP crowdsafe_step_count Total simulation steps completed
# TYPE crowdsafe_step_count counter
crowdsafe_step_count 1500
# HELP crowdsafe_n_pedestrians Current number of pedestrians
# TYPE crowdsafe_n_pedestrians gauge
crowdsafe_n_pedestrians 100
# HELP crowdsafe_mean_speed_kmh Mean pedestrian speed in km/h
# TYPE crowdsafe_mean_speed_kmh gauge
crowdsafe_mean_speed_kmh 72.45
# HELP crowdsafe_rate_limit_keys Active rate limiter keys
# TYPE crowdsafe_rate_limit_keys gauge
crowdsafe_rate_limit_keys 3
```
...
| `crowdsafe_rate_limit_keys` | gauge | Number of active rate limiter tracking keys |
...
| `n_pedestrians` | int | `100` | 1--100,000 | Number of pedestrians |
...
| `mean_speed_kmh` | float | Mean speed of all pedestrians at prediction time |
| `n_pedestrians` | int | Number of pedestrians |
| `pedestrians` | object[] | Predicted state of each pedestrian |

**Error** `409 Conflict`: No simulation created.

> **Note:** Prediction with `horizon_s=900` (15 minutes) at `dt=0.1` executes 9000 steps on the server. Expect response times of several seconds depending on `n_pedestrians`.
...
1. On connect, the server auto-creates a default simulation (5x5 grid, 100 pedestrians) if none exists via REST.
...
```json
{"type": "config_ok", "n_pedestrians": 500}
```
...
| `pedestrians` | object[] | Current state of each pedestrian |
| `kpi.mean_speed_kmh` | float | Mean speed across all pedestrians |
| `kpi.congestion_index` | float | Fraction of pedestrians classified as "slow" (positive mass) |
| `kpi.n_pedestrians` | int | Total pedestrian count |
