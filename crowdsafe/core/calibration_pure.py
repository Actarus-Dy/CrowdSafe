...
At equilibrium (all pedestrians at v_mean), ALL masses are zero and ALL forces vanish.
This is a fixed point. The gravitational force acts as a RESTORING force that pushes
deviating pedestrians back toward the mean speed. But it does NOT determine what the mean
...
1. For each density rho in [10, 20, ..., 140] veh/km:
   - Place N = rho * L pedestrians uniformly on a 2 km segment
...
Reference
---------
Janus Civil C-01 CrowdSafe Technical Plan, Section 5 (Calibration).
...
def _setup_scenario(
    ...
    ----------
    rho_veh_per_km : float
        Target density in pedestrians per km.
    v_free : float
    ...
    """
    # Number of pedestrians on our 2 km segment
    n_pedestrians = max(2, int(round(rho_veh_per_km * SEGMENT_LENGTH_M / 1000.0)))

    # Uniform spacing
    spacing = SEGMENT_LENGTH_M / n_pedestrians
    positions_x = np.arange(n_pedestrians, dtype=np.float64) * spacing + spacing * 0.5
    ...
    noise = rng.normal(0.0, NOISE_SIGMA, size=n_pedestrians)
    ...
    # Local density: all pedestrians see the same uniform density
    local_densities = np.full(n_pedestrians, rho_veh_per_km, dtype=np.float64)
...
def run_pure_gravity_test(
    ...
    For each density, creates a uniform pedestrian distribution at the Greenshields
    ...
    for idx, rho in enumerate(densities):
        ...
        sim.init_pedestrians(positions, velocities, local_dens)
...
def run_generation_test(
    ...
    this test starts ALL pedestrians at v_free regardless of density. If the
    gravitational model can generate a fundamental diagram, pedestrians at high
    density should slow down and pedestrians at low density should stay fast.
    ...
    for idx, rho in enumerate(densities):
        n_pedestrians = max(2, int(round(rho * SEGMENT_LENGTH_M / 1000.0)))
        spacing = SEGMENT_LENGTH_M / n_pedestrians
        positions_x = np.arange(n_pedestrians, dtype=np.float64) * spacing + spacing * 0.5
        ...
        # KEY DIFFERENCE: all pedestrians start at v_free + small noise
        noise = rng.normal(0.0, NOISE_SIGMA, size=n_pedestrians)
        ...
        local_dens = np.full(n_pedestrians, rho, dtype=np.float64)
    ...
    # Speed reduction from v_free -- does gravity slow pedestrians at high density?
...
def _print_physics_explanation() -> None:
    ...
    print("  2. Gravity acts as a RESTORING force that pushes pedestrians back toward")
...
def _print_enrichment_suggestions() -> None:
    ...
    print("  the inter-pedestrian interaction dynamics (clustering, platoon formation, etc.)")
    print()
