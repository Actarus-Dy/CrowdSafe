"""Traffic metrics and KPI calculator for CrowdSafe (C-01).

This module provides fully vectorized NumPy functions that compute traffic
performance indicators from pedestrian state arrays.  No Python-level loops
iterate over pedestrians -- all hot paths use broadcasting and array operations.

Metrics implemented
-------------------
- **Throughput** -- pedestrians crossing a counting gate per hour.
- **Mean speed** -- population-average scalar speed [m/s].
- **Delay** -- excess travel time relative to free-flow speed [s/km].
- **Stops** -- count of pedestrians below a speed threshold.
- **Congestion index** -- fraction of pedestrians with positive gravitational mass
...
- **Travel time** -- mean time for pedestrians to traverse a corridor segment.
- **Snapshot KPIs** -- convenience dict aggregating all instantaneous metrics.

Reference
---------
Janus Civil C-01 CrowdSafe Technical Plan, Section 6 -- Performance Metrics.
...
def compute_throughput(
    ...
    """Count pedestrians crossing *gate_x* between two timesteps.

    A crossing is detected when a pedestrian's x-coordinate moves from one
    ...
    -------
    float
        Throughput in pedestrians per hour.
...
def compute_mean_speed(velocities: npt.NDArray[np.float64]) -> float:
    ...
    velocities : ndarray, shape (N, 2), dtype float64
        Velocity vectors (vx, vy) of each pedestrian.
...
def compute_delay(
    ...
    """Mean delay per pedestrian [seconds per kilometre].

    Delay is the excess travel time compared to free-flow conditions:

        delay_i = 1000 / v_i  -  1000 / v_free

    Pedestrian speeds are clamped to a minimum of 0.1 m/s to avoid division
    by zero.
...
def compute_stops(
    ...
    """Count pedestrians whose scalar speed is below *threshold*.
    ...
    threshold : float, default 2.0
        Speed below which a pedestrian is considered stopped [m/s].

    Returns
    -------
    int
        Number of stopped pedestrians.
    """
    velocities = np.asarray(velocities, dtype=np.float64)
    if velocities.size == 0:
        return 0
...
def compute_congestion_index(masses: npt.NDArray[np.float64]) -> float:
    """Fraction of pedestrians with positive gravitational mass (slow/congested).

    A pedestrian is considered congested when its mass exceeds 0.1, consistent
...
def compute_travel_time(
    ...
    """Mean travel time for pedestrians traversing from *start_x* to *end_x*.

    The function scans the time series of position snapshots to determine
    when each pedestrian first reaches ``start_x`` (entry) and ``end_x``
    (exit).  Only pedestrians that complete the full trip contribute to the
    average.

    Per-timestep logic is vectorized over pedestrians (no inner Python loop
    on the pedestrian dimension).  The outer loop over timesteps is
    ...
    -------
    float or None
        Mean travel time [seconds], or ``None`` if no pedestrian completed
        the trip.
    """
    if len(positions_history) == 0:
        return None

    n_pedestrians = positions_history[0].shape[0]
    if n_pedestrians == 0:
        return None

    entry_step = np.full(n_pedestrians, -1, dtype=np.int64)
    exit_step = np.full(n_pedestrians, -1, dtype=np.int64)
