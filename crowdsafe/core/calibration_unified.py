...
that simultaneously:
  ...
  A. **Calibration**: Reproduce a Greenshields speed-density fundamental diagram
     when starting pedestrians at free-flow speed and letting the drag term pull
  ...
  B. **Emergence**: Produce measurable upstream deceleration when a single slow
     pedestrian is injected into a uniform stream.  The gravitational interaction
...
def run_calibration_test(
    ...
    Pedestrians start at free-flow speed (v_free) at various densities.
    ...
    for idx, rho in enumerate(densities):
        ...
        # Start ALL pedestrians at v_free (generation test, not preservation)
        noise = rng.normal(0.0, 0.5, size=n_pedestrians)
...
# ---------------------------------------------------------------------------
# Test B: Emergence -- upstream deceleration from slow pedestrian
# ---------------------------------------------------------------------------
def run_emergence_test(
    ...
    """Test emergence: does a slow pedestrian cause upstream deceleration?
    ...
    initial_speed : float
        Initial speed for all pedestrians (m/s).
    slow_speed : float
        Speed of the injected slow pedestrian (m/s).
    n_steps : int
        Simulation steps.
    dt : float
        Fixed timestep.
    ...
    """
    np.random.seed(seed)
    spacing = highway_length / n_pedestrians
    ...
    # Inject slow pedestrian at midpoint
    slow_idx = int(np.argmin(np.abs(x_positions - highway_length / 2)))
    velocities[slow_idx, 0] = slow_speed

    # Density ~ n_pedestrians / (highway_length / 1000) veh/km
    density = n_pedestrians / (highway_length / 1000.0)
    local_densities = np.full(n_pedestrians, density, dtype=np.float64)
    ...
    # Identify upstream pedestrians: originally at x in [700, 950]
    init_x = x_positions
    ...
    # Downstream pedestrians: originally at x in [1050, 1300]
