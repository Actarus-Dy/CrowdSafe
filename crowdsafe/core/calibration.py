...
def _estimate_local_density(
    ...
    Divides the segment into *n_bins* equal bins and counts pedestrians in
    each bin, then converts to veh/km.

    Parameters
    ----------
    positions_x : ndarray, shape (N,)
        Pedestrian x-positions in meters along the segment.
    ...
    -------
    ndarray, shape (N,), dtype float64
        Local density at each pedestrian's position, in veh/km.
    """
...
def run_calibration_test(
    ...
    3. Place pedestrians on a 1D segment with spacing derived from density.
    ...
    beta : float
        Mass exponent.
    n_pedestrians : int
        Number of synthetic pedestrians (default 100).
    n_steps : int
    ...
    # ------------------------------------------------------------------
    densities = rng.uniform(5.0, 140.0, size=n_pedestrians).astype(np.float64)

    # ------------------------------------------------------------------
    # Step 2: Greenshields speeds with noise
    # ------------------------------------------------------------------
    analytical_speeds = _greenshields_speed(densities)
    noisy_speeds = analytical_speeds + rng.normal(0.0, NOISE_SIGMA, size=n_pedestrians)
    # Clamp speeds to physical range [0, v_free]
    noisy_speeds = np.clip(noisy_speeds, 0.0, V_FREE_MS).astype(np.float64)

    # ------------------------------------------------------------------
    # Step 3: Place pedestrians on 1D segment
    # ------------------------------------------------------------------
    # Position each pedestrian based on its "slot" within the segment.
    # For a pedestrian at density rho, the average spacing is 1000/rho meters.
    # We place pedestrians sorted by density-derived position to create a
    # realistic spatial distribution.
    sort_idx = np.argsort(densities)
    # Cumulative spacing: each pedestrian gets 1000/rho_i meters of headway
    spacings = 1000.0 / densities[sort_idx]  # meters per pedestrian
    ...
    positions_x = np.empty(n_pedestrians, dtype=np.float64)
    ...
    for step in range(n_steps):
        ...
        # densities, because each pedestrian's "desired" speed is set by
