...
- Pedestrians injected at both ends at a steady rate

The scenario uses CrowdSimulation directly with manual intersection management
(no CrowdModel / Mesa overhead).
...
from crowdsafe.core.green_wave import GreenWaveCoordinator
from crowdsafe.core.metrics import (
    compute_mean_speed,
    compute_stops,
    compute_throughput,
)
from crowdsafe.core.signal_optimizer import optimize_signal_timing
...
class RivoliCorridor:
    ...
    injection_rate : float
        Pedestrian injection rate per direction [pedestrians/second].
    seed : int
    ...
    def run_fixed_timing(
        ...
        -------
        dict
            KPI dictionary with mode, speeds, stops, pedestrian counts.
        """
    ...
    def _run(self, duration_s: float, mode: str, **kwargs) -> dict:
        ...
        **kwargs
        ...
        # --- Initialize pedestrians on the corridor ---
        # ~1 pedestrian per 20 m, split roughly 50/50 eastbound/westbound
        ...
        # --- Create simulation ---
        sim = CrowdSimulation(**self.sim_params)
        ...
        for step in range(n_steps):
            # --- Inject pedestrians at corridor ends ---
            if rng.random() < self.injection_rate * dt:
                ...
                sim.add_pedestrians(new_pos, new_vel, np.array([50.0]))
            ...
            if mode == "optimized" and step > 0 and step % opt_interval == 0 and sim.n_pedestrians > 0:
                for i in range(self.n_intersections):
                    ix = float(self.intersection_x[i])
        ...
        return {
            ...
            "final_n_pedestrians": sim.n_pedestrians,
            "total_steps": n_steps,
        }
