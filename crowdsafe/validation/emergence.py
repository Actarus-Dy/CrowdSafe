...
2. Gini coefficient of pedestrian spacings (clustering)
...
def run_emergence_analysis(
    ...
    """
    pos, vel, dens, slow_idx = _build_scenario(n_pedestrians=n_pedestrians, seed=seed)
    ...
    for label, g_s in [("gravity_on", G_s), ("gravity_off", 0.0)]:
        ...
        sim.init_pedestrians(pos.copy(), vel.copy(), dens.copy())
        ...
        for step in range(n_steps):
            ...
            # Pedestrian spacings (sorted by x)
        ...
        # Upstream deceleration: pedestrians approaching from behind (x < slow_x)
        # in a flow moving toward +x. These are the pedestrians that should
        ...
        # Wave speed estimation: find the furthest upstream pedestrian that decelerated
