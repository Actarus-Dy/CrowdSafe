"""CrowdSafe FastAPI application -- REST + WebSocket API.
...
class SimulationConfig(BaseModel):
    ...
    n_pedestrians: int = Field(default=100, ge=1, le=100_000, description="Number of pedestrians")
...
class AppState:
    ...
    def __init__(self) -> None:
        self.model: CrowdModel | None = None
...
app = FastAPI(
    ...
    version="0.2.0",
    description=(
        "REST and WebSocket API for real-time gravitational crowd simulation "
        "based on the Janus Cosmological Model (C-01).\n\n"
        "**Core physics:** pedestrians have positive mass (slow, attract, jam) or "
...
def _build_model(config: SimulationConfig) -> CrowdModel:
    """Construct a CrowdModel from a SimulationConfig."""
...
async def ready() -> dict[str, Any]:
    ...
    async with state.lock:
        ...
        n = len(state.model.pedestrian_agents)
    return {"status": "ready", "n_pedestrians": n}
...
async def metrics():
    ...
    async with state.lock:
        if state.model is not None:
            ...
            n_veh = len(state.model.pedestrian_agents)
...
async def get_status() -> dict[str, Any]:
    ...
    async with state.lock:
        if state.model is None:
            return {"running": False, "step": 0, "n_pedestrians": 0}
        return {
            "running": state.running,
            "step": state.model.step_count,
            "n_pedestrians": len(state.model.pedestrian_agents),
        }
...
async def create_simulation(config: SimulationConfig) -> dict[str, Any]:
    ...
    async with state.lock:
        ...
        state.config = config
        state.running = False
    return {"status": "created", "n_pedestrians": config.n_pedestrians}
...
async def stream_potential(websocket: WebSocket) -> None:
    ...
    Server frames (JSON)
    --------------------
    Each frame has ``type: "frame"`` plus ``step``, ``pedestrians``,
        ...
        while True:
                ...
                if msg_type == "stop":
                    ...
                    try:
                        ...
                        await websocket.send_json(
                            {"type": "config_ok", "n_pedestrians": new_cfg.n_pedestrians}
                        )
            ...
            if state.running and state.model is not None:
                async with state.lock:
                    ...
                    pedestrians = [a.to_dict() for a in state.model.pedestrian_agents]
                ...
                frame: dict[str, Any] = {
                    ...
                    "kpi": {
                        "mean_speed_kmh": float(
                            np.mean([v["speed_kmh"] for v in pedestrians]) if pedestrians else 0.0
                        ),
                        "congestion_index": float(
                            sum(1 for v in pedestrians if v["type"] == "slow") / max(len(pedestrians), 1)
                        ),
                        "n_pedestrians": len(pedestrians),
                    },
                }
