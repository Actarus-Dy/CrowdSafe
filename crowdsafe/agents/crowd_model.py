"""CrowdModel -- Mesa Model orchestrating CrowdSimulation and agents.

This is the top-level simulation object for C-01 CrowdSafe.  It coordinates
data flow between three subsystems:

1. **CrowdSimulation** (physics engine):  mass assignment, Barnes-Hut force
   computation, leapfrog integration.  All numerics live here.
2. **PedestrianAgent** (per-pedestrian state):  holds position, velocity, mass for
...
2. Run one physics step via ``CrowdSimulation.step()``.
3. Push updated positions, velocities, and masses to PedestrianAgents.
...
from crowdsafe.core.potential_field import compute_potential_field, make_grid
...
class CrowdModel(mesa.Model):
    """CrowdSafe Mesa Model -- orchestrates physics simulation and agents.
    ...
    Attributes
    ----------
    pedestrian_agents : list[PedestrianAgent]
        All pedestrian agents, index-aligned with the physics engine arrays.
    ...
    def __init__(
        ...
        # ---- Physics engine ------------------------------------------------
        self.simulation = CrowdSimulation(
            G_s=G_s,
            beta=beta,
            softening=softening,
            theta=theta,
        ...
        # ---- Spawn pedestrians on the network ---------------------------------
        rng = np.random.default_rng(seed)
        positions = network.sample_positions(n_pedestrians, rng=rng)
    ...
    def spawn_pedestrian(
        ...
        """Spawn a single pedestrian and add it to both simulation and agent list.
        ...
        self.simulation.add_pedestrians(
    ...
    def despawn_pedestrian(self, index: int) -> None:
        """Remove a pedestrian by index from both simulation and agent list.

        Parameters
        ----------
        index : int
            Index of the pedestrian to remove (into ``pedestrian_agents`` and the
            physics engine arrays, which are kept index-aligned).
        """
        self.simulation.remove_pedestrians(np.array([index], dtype=np.intp))
        self.pedestrian_agents.pop(index)
    ...
    def despawn_out_of_bounds(
        ...
        """Remove all pedestrians outside a bounding box.
        ...
        self.simulation.remove_pedestrians(indices)

        # Remove agents in reverse order to preserve earlier indices
        for i in sorted(indices, reverse=True):
            self.pedestrian_agents.pop(i)
    ...
    def step(self) -> None:
        ...
        #    them into CrowdSimulation as static obstacles.  Positive masses
        #    repel fast pedestrians (deceleration) and attract slow pedestrians
        ...
        # 3. Push updated state to PedestrianAgents
        positions = result["positions"]
        velocities = result["velocities"]
        masses = result["masses"]

        for i, agent in enumerate(self.pedestrian_agents):
        ...
        for ia in self.intersection_agents:
            ia.step(dt=dt_used)
            ia.try_optimize(positions, masses, G_s=self.G_s, pedestrian_velocities=velocities)
    ...
    def get_potential_field(self, resolution: float = 10.0) -> dict:
        ...
        """
        positions = np.array([a.position for a in self.pedestrian_agents], dtype=np.float64)
        masses = np.array([a.mass for a in self.pedestrian_agents], dtype=np.float64)
    ...
    def get_state(self) -> dict:
        ...
        -------
        dict
            Keys: ``'step'``, ``'pedestrians'`` (list of dicts),
        ...
        return {
            ...
            "pedestrians": [a.to_dict() for a in self.pedestrian_agents],
...
def _mean_speed_kmh(model: CrowdModel) -> float:
    """Mean pedestrian speed in km/h across all agents."""
    if not model.pedestrian_agents:
        return 0.0
    return float(np.mean([a.speed_kmh for a in model.pedestrian_agents]))


def _congestion_index(model: CrowdModel) -> float:
    """Fraction of pedestrians classified as 'slow' (positive-mass attractors).

    Returns a value in [0, 1].  A value of 0 means no congestion seeds;
    a value of 1 means every pedestrian is slow relative to the mean flow.
    """
    n = len(model.pedestrian_agents)
    if n == 0:
        return 0.0
    n_slow = sum(1 for a in model.pedestrian_agents if a.mass_type == "slow")
    return float(n_slow / n)


def _n_slow(model: CrowdModel) -> int:
    """Number of pedestrians with mass_type == 'slow'."""
    return sum(1 for a in model.pedestrian_agents if a.mass_type == "slow")


def _n_fast(model: CrowdModel) -> int:
    """Number of pedestrians with mass_type == 'fast'."""
    return sum(1 for a in model.pedestrian_agents if a.mass_type == "fast")


def _n_neutral(model: CrowdModel) -> int:
    """Number of pedestrians with mass_type == 'neutral'."""
    return sum(1 for a in model.pedestrian_agents if a.mass_type == "neutral")
