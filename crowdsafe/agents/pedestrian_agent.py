"""PedestrianAgent -- Mesa ABM agent for CrowdSafe simulation.

Each pedestrian holds its kinematic state (position, velocity) and its
gravitational mass as computed by the central :class:`CrowdSimulation` engine.
...
:mod:`crowdsafe.core.mass_assigner`:
...
Reference
---------
Janus Civil C-01 CrowdSafe Technical Plan, Section 5.1.
"""
...
__all__ = ["PedestrianAgent"]

# Must match crowdsafe.core.mass_assigner._NEUTRAL_THRESHOLD
_NEUTRAL_THRESHOLD: float = 0.1


class PedestrianAgent(mesa.Agent):
    """A pedestrian in the CrowdSafe simulation.

    This agent does **not** compute its own forces or make behavioural
    decisions.  All physics is computed centrally by ``CrowdSimulation``,
    ...
    def update_from_simulation(
        ...
        """Receive updated state from the central CrowdSimulation engine.
    ...
    def __repr__(self) -> str:
        return (
            f"PedestrianAgent(id={self.unique_id}, "
