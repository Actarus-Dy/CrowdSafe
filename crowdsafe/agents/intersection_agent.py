"""IntersectionAgent -- signalized intersection for the CrowdSafe ABM.
...
Physics analogy (Janus C-01, section 2.4):
    ...
    pedestrians.  When the light turns green the mass is removed and pedestrians
...
class IntersectionAgent(mesa.Agent):
    """A signalized intersection in the CrowdSafe simulation.
    ...
    def try_optimize(
        ...
        G_s: float = 5.0,
        pedestrian_velocities: npt.NDArray[np.float64] | None = None,
    ) -> dict | None:
        """Re-optimize green timings if the optimization interval has elapsed.

        Calls :func:`crowdsafe.core.signal_optimizer.optimize_signal_timing`
        with the current pedestrian state to determine new green-phase durations
        ...
        pedestrian_masses : ndarray, shape (N,), dtype float64
            Signed gravitational masses of all pedestrians.
        G_s : float, default 5.0
            Social gravitational constant passed to the optimizer
            (unified calibration).
        pedestrian_velocities : ndarray, shape (N, 2), dtype float64, optional
            Current (vx, vy) velocities of all pedestrians.  If *None*, zero
        ...
        if self.steps_since_optimize >= self.optimize_interval_steps:
            ...
            n = len(pedestrian_masses)
            if pedestrian_velocities is None:
                pedestrian_velocities = np.zeros((n, 2), dtype=np.float64)
