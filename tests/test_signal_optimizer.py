"""Tests for crowdsafe.core.signal_optimizer -- time-integrated potential optimizer.
...
from crowdsafe.core.signal_optimizer import (
...
def symmetric_traffic(intersection_pos: np.ndarray) -> dict:
    """10 NS pedestrians + 10 EW pedestrians, symmetric about the intersection.

    All pedestrians are slow (positive mass) and approaching at 10 m/s.
    """
    rng = np.random.default_rng(42)

    # NS pedestrians: approaching from +y and -y
    ...
    # EW pedestrians: approaching from +x and -x
...
class TestEstimatePhiIntegral:
    ...
    # -------------------------------------------------------------------
    # Test 6: No pedestrians in radius -> phi = 0
    # -------------------------------------------------------------------

    def test_no_pedestrians_in_radius(self, intersection_pos: np.ndarray) -> None:
        """When no pedestrians are within the radius, phi integral must be 0.0."""
        # Pedestrians are 500 m away, radius is 200 m
    ...
    def test_no_pedestrians_at_all(self, intersection_pos: np.ndarray) -> None:
        """Empty pedestrian arrays must produce phi = 0.0."""
        phi = estimate_phi_integral(
...
class TestOptimizeSignalTiming:
    ...
    def test_asymmetric_ns_heavy_favours_longer_green_ns(
        ...
        We place 30 slow (positive mass) NS pedestrians and only 2 EW pedestrians.
        """
        rng = np.random.default_rng(123)

        # 30 NS pedestrians approaching from +y
        ns_pos = np.column_stack(
            [
                rng.uniform(-5, 5, 30),
        ...
        # 2 EW pedestrians (light traffic)
    ...
    def test_no_nearby_pedestrians(self, intersection_pos: np.ndarray) -> None:
        """With no pedestrians in radius, phi values should be zero and
...
class TestVelocityDependence:
    ...
    The signal_optimizer uses linear extrapolation of pedestrian positions.
    With zero velocities, pedestrians never move and the extrapolation is
    trivially static.  With real approaching velocities, pedestrians reach
    ...
    def test_phi_integral_differs_with_real_vs_zero_velocities(
        ...
        pedestrians vs stationary pedestrians."""
        # Pedestrians approaching the intersection from +y at 10 m/s
        positions = np.array(
            [
                [0.0, 80.0],
                [0.0, 120.0],
    ...
    def test_approaching_pedestrians_produce_stronger_potential(
        self, intersection_pos: np.ndarray
    ) -> None:
        """Pedestrians moving toward the intersection should produce a more
        ...
        # Approaching pedestrians get closer to obstacles -> stronger (more negative) phi
        assert phi_approach < phi_static, (
            f"Approaching pedestrians should produce more negative phi integral, "
    ...
    def test_optimizer_uses_velocity_information(self, intersection_pos: np.ndarray) -> None:
        ...
        # 20 NS pedestrians approaching fast, 5 EW pedestrians stationary
        ns_pos = np.column_stack(
            [
                rng.uniform(-5, 5, 20),
        ...
        # The phi integrals should differ since pedestrian trajectories differ
