...
class TestSimulationGPUIntegration:
    """Test that CrowdSimulation correctly uses GPU when available."""

    def test_simulation_with_gpu_flag(self) -> None:
        from crowdsafe.core.simulation import CrowdSimulation

        sim = CrowdSimulation(G_s=5.0, use_gpu=True)
        if GPU_AVAILABLE:
    ...
    def test_simulation_with_gpu_false(self) -> None:
        ...
        sim = CrowdSimulation(G_s=5.0, use_gpu=False)
        assert not isinstance(sim._force_engine, ForceEngineGPU)
    ...
    def test_gpu_fallback_without_cupy(self) -> None:
        ...
        sim = CrowdSimulation(G_s=5.0, use_gpu=False, adaptive_dt=False, dt=0.1)
        assert not isinstance(sim._force_engine, ForceEngineGPU)
    ...
    def test_simulation_gpu_false_runs_step(self) -> None:
        ...
        sim.init_pedestrians(pos, vel, np.full(n, 50.0, dtype=np.float64))
        result = sim.step()
