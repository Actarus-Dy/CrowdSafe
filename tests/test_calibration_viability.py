"""Calibration viability tests -- go/no-go gate for CrowdSafe (C-01).
...
from crowdsafe.core.calibration import (
    RHO_JAM,
    V_FREE_MS,
    _compute_r_squared,
    _compute_rmse,
...
class TestRunCalibrationTest:
    ...
    def test_returns_valid_dict_with_all_keys(self) -> None:
        ...
        result = run_calibration_test(G_s=9.8, beta=1.0, n_pedestrians=30, n_steps=10)
    ...
    def test_output_array_shapes(self) -> None:
        """final_speeds and densities must have shape (n_pedestrians,)."""
        n = 50
        result = run_calibration_test(G_s=2.0, beta=0.5, n_pedestrians=n, n_steps=5)
    ...
    def test_output_dtype_float64(self) -> None:
        ...
        result = run_calibration_test(G_s=2.0, beta=0.5, n_pedestrians=30, n_steps=5)
    ...
    def test_speeds_physically_bounded(self) -> None:
        ...
        result = run_calibration_test(G_s=15.0, beta=1.5, n_pedestrians=50, n_steps=50)
    ...
    def test_densities_in_expected_range(self) -> None:
        ...
        result = run_calibration_test(G_s=9.8, beta=1.0, n_pedestrians=100, n_steps=10)
    ...
    def test_g_s_and_beta_echoed(self) -> None:
        ...
        result = run_calibration_test(G_s=2.0, beta=0.5, n_pedestrians=20, n_steps=5)
        assert result["G_s"] == 2.0
        assert result["beta"] == 0.5

    def test_reproducibility_with_seed(self) -> None:
        """Same seed produces identical results."""
        r1 = run_calibration_test(G_s=9.8, beta=1.0, n_pedestrians=30, n_steps=10, seed=42)
        r2 = run_calibration_test(G_s=9.8, beta=1.0, n_pedestrians=30, n_steps=10, seed=42)
        assert r1["r_squared"] == r2["r_squared"]
        np.testing.assert_array_equal(r1["final_speeds"], r2["final_speeds"])

    def test_different_seeds_differ(self) -> None:
        """Different seeds produce different results."""
        r1 = run_calibration_test(G_s=9.8, beta=1.0, n_pedestrians=50, n_steps=10, seed=42)
        r2 = run_calibration_test(G_s=9.8, beta=1.0, n_pedestrians=50, n_steps=10, seed=99)
...
class TestModelSanity:
    ...
    def test_speeds_vary_with_density(self) -> None:
        ...
        result = run_calibration_test(G_s=9.8, beta=1.0, n_pedestrians=100, n_steps=50)
        speed_std = np.std(result["final_speeds"])
        assert speed_std > 0.5, (
            f"Speed standard deviation = {speed_std:.4f} m/s is too small. "
            "The model may have collapsed all pedestrians to one speed."
        )
...
class TestEdgeCases:
    ...
    def test_single_pedestrian(self) -> None:
        """With 1 pedestrian, no N-body forces act; speed changes only via relaxation."""
        result = run_calibration_test(G_s=9.8, beta=1.0, n_pedestrians=1, n_steps=10)
        assert result["final_speeds"].shape == (1,)
        assert np.isfinite(result["final_speeds"][0])

    def test_two_pedestrians(self) -> None:
        """Minimal N-body: 2 pedestrians interact."""
        result = run_calibration_test(G_s=9.8, beta=1.0, n_pedestrians=2, n_steps=10)
    ...
    def test_zero_g_s(self) -> None:
        ...
        result = run_calibration_test(G_s=0.0, beta=1.0, n_pedestrians=50, n_steps=50)
    ...
    def test_very_large_g_s(self) -> None:
        ...
        result = run_calibration_test(G_s=1000.0, beta=1.0, n_pedestrians=30, n_steps=10)
        assert np.all(np.isfinite(result["final_speeds"]))

    def test_beta_zero(self) -> None:
        """beta=0: all |delta|^0 = 1, mass depends only on sign and density."""
        result = run_calibration_test(G_s=9.8, beta=0.0, n_pedestrians=30, n_steps=10)
        assert np.all(np.isfinite(result["final_speeds"]))
        assert np.isfinite(result["r_squared"])
