"""Integration tests for CrowdSimulation pipeline.

Tests
-----
1. CrowdSimulation with 50 pedestrians runs 10 steps without error.
...
6. Pedestrians do not exceed v_max after stepping.
...
# ---------------------------------------------------------------------------
# Test 1: 50 pedestrians, 10 steps, no error
# ---------------------------------------------------------------------------
class TestSimulationRunsWithoutError:
    """CrowdSimulation with 50 pedestrians runs 10 steps without error."""

    def test_10_steps_no_exception(self, sim_50: CrowdSimulation) -> None:
        for _ in range(10):
            sim_50.step()
        assert sim_50.step_count == 10

    def test_positions_remain_finite(self, sim_50: CrowdSimulation) -> None:
        for _ in range(10):
            sim_50.step()
        assert np.all(np.isfinite(sim_50.positions))

    def test_velocities_remain_finite(self, sim_50: CrowdSimulation) -> None:
        for _ in range(10):
            sim_50.step()
...
class TestMassAssignment:
    """Slow pedestrians get positive mass, fast pedestrians get negative mass."""
    ...
    def test_slow_pedestrians_positive_mass(self) -> None:
        ...
        assert sim.masses[0] > 0.0, "Slow pedestrian should have positive mass"

    def test_fast_pedestrians_negative_mass(self) -> None:
        """A pedestrian much faster than the mean should have negative mass."""
        n = 20
        positions = np.column_stack(
            [
                np.linspace(0, 30, n),
...
class TestStepReturnDict:
    ...
    def test_keys_present(self, sim_50: CrowdSimulation) -> None:
        result = sim_50.step()
        assert set(result.keys()) == self._EXPECTED_KEYS

    def test_positions_shape(self, sim_50: CrowdSimulation) -> None:
        result = sim_50.step()
        assert result["positions"].shape == (50, 2)

    def test_velocities_shape(self, sim_50: CrowdSimulation) -> None:
        result = sim_50.step()
        assert result["velocities"].shape == (50, 2)

    def test_masses_shape(self, sim_50: CrowdSimulation) -> None:
        result = sim_50.step()
        assert result["masses"].shape == (50,)

    def test_mean_speed_is_float(self, sim_50: CrowdSimulation) -> None:
        result = sim_50.step()
        assert isinstance(result["mean_speed"], float)

    def test_dt_used_positive(self, sim_50: CrowdSimulation) -> None:
        result = sim_50.step()
        assert result["dt_used"] > 0.0

    def test_step_count_increments(self, sim_50: CrowdSimulation) -> None:
        r1 = sim_50.step()
        r2 = sim_50.step()
        assert r1["step_count"] == 1
        assert r2["step_count"] == 2

    def test_return_arrays_are_copies(self, sim_50: CrowdSimulation) -> None:
...
class TestPotentialField:
    ...
    def test_potential_field_shape(self, sim_50: CrowdSimulation) -> None:
        grid = np.array([[100.0, 0.0], [200.0, 0.0], [300.0, 0.0]])
        phi = sim_50.get_potential_field(grid)
        assert phi.shape == (3,)

    def test_potential_field_finite(self, sim_50: CrowdSimulation) -> None:
        grid = np.array([[100.0, 0.0], [200.0, 0.0], [300.0, 0.0]])
        phi = sim_50.get_potential_field(grid)
        assert np.all(np.isfinite(phi))

    def test_potential_field_after_stepping(self, sim_50: CrowdSimulation) -> None:
...
class TestRunMethod:
    ...
    def test_run_length(self, sim_50: CrowdSimulation) -> None:
        results = sim_50.run(7)
        assert len(results) == 7

    def test_run_step_counts_sequential(self, sim_50: CrowdSimulation) -> None:
        results = sim_50.run(5)
        for i, r in enumerate(results, start=1):
            assert r["step_count"] == i

    def test_run_zero_steps(self, sim_50: CrowdSimulation) -> None:
...
class TestSpeedLimiter:
    """Pedestrians do not exceed v_max after stepping."""
...
class TestDtypeEnforcement:
    ...
    def test_positions_dtype(self, sim_50: CrowdSimulation) -> None:
        sim_50.step()
        assert sim_50.positions.dtype == np.float64

    def test_velocities_dtype(self, sim_50: CrowdSimulation) -> None:
        sim_50.step()
        assert sim_50.velocities.dtype == np.float64

    def test_masses_dtype(self, sim_50: CrowdSimulation) -> None:
        sim_50.step()
...
class TestLocalDensitiesUpdate:
    ...
    def test_local_densities_update(self, sim_50: CrowdSimulation) -> None:
        ...
        # Densities must have changed (pedestrians moved, so neighborhoods change)
    ...
    def test_local_densities_shape(self, sim_50: CrowdSimulation) -> None:
...
# ---------------------------------------------------------------------------
class TestObstacles:
    """Static obstacles (e.g. red-light masses) affect pedestrian forces."""

    @staticmethod
    def _make_sim_with_fast_pedestrian() -> CrowdSimulation:
        """Helper: create a simulation with one fast pedestrian (negative mass)
    ...
    def test_obstacles_affect_forces(self) -> None:
        ...
        Uses a population with clear fast/slow split so pedestrians have
        ...
        assert not np.allclose(result_a["velocities"], result_b["velocities"]), (
            "Obstacle had no effect on pedestrian velocities"
        )
    ...
    def test_obstacle_affects_approaching_pedestrian(self) -> None:
        ...
        assert sim_free.masses[0] < 0.0, "Pedestrian 0 should have negative mass"
        for _ in range(20):
            sim_free.step()
