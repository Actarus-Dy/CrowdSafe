"""Tests for the CrowdSafe FastAPI application.
...
from crowdsafe.api.app import app, state
...
class TestCreateSimulation:
    ...
    def test_create_replaces_previous(self, client: TestClient) -> None:
        client.post("/api/v1/simulate", json=DEFAULT_CONFIG)
        new_cfg = {**DEFAULT_CONFIG, "n_pedestrians": 10}
        resp = client.post("/api/v1/simulate", json=new_cfg)
        assert resp.status_code == 201
        assert resp.json()["n_pedestrians"] == 10
        # Status reflects the new simulation
        assert client.get("/api/v1/status").json()["n_pedestrians"] == 10
...
class TestStepSimulation:
    ...
    def test_step_returns_state(self, client: TestClient) -> None:
        ...
        assert isinstance(body["pedestrians"], list)
        assert len(body["pedestrians"]) == 20
    ...
    def test_pedestrian_dict_structure(self, client: TestClient) -> None:
        client.post("/api/v1/simulate", json=DEFAULT_CONFIG)
        resp = client.post("/api/v1/step")
        v = resp.json()["pedestrians"][0]
        for key in ("id", "x", "y", "vx", "vy", "mass", "speed_kmh", "type"):
            assert key in v, f"Missing key {key!r} in pedestrian dict"
...
class TestWebSocketStream:
    def test_receive_frames(self, client: TestClient) -> None:
        ...
        with client.websocket_connect("/ws/stream/potential") as ws:
            ...
            for frame in frames:
                ...
                assert "n_pedestrians" in frame["kpi"]
    ...
    def test_auto_creates_simulation(self) -> None:
        ...
        with test_client.websocket_connect("/ws/stream/potential") as ws:
            ...
            assert frame["type"] == "frame"
            assert frame["kpi"]["n_pedestrians"] == 100  # default config
...
class TestInputValidation:
    ...
    def test_invalid_n_pedestrians_zero(self, client: TestClient) -> None:
        """n_pedestrians=0 should be rejected."""
        bad = {**DEFAULT_CONFIG, "n_pedestrians": 0}
...
class TestHealthEndpoints:
    ...
    def test_ready_200_with_simulation(self, client: TestClient) -> None:
        ...
        assert resp.json()["n_pedestrians"] == 20
    ...
    def test_metrics_returns_prometheus_format(self, client: TestClient) -> None:
        ...
        assert resp.status_code == 200
        body = resp.text
        assert "crowdsafe_uptime_seconds" in body
        assert "crowdsafe_step_count 1" in body
        assert "crowdsafe_n_pedestrians 20" in body
        assert "crowdsafe_mean_speed_kmh" in body

    def test_metrics_without_simulation(self, client: TestClient) -> None:
        resp = client.get("/metrics")
        assert resp.status_code == 200
        assert "crowdsafe_step_count 0" in resp.text
...
class TestRateLimiting:
    def test_rate_limiter_allows_normal_traffic(self, client: TestClient) -> None:
        ...
        from crowdsafe.api.app import _rate_limiter
