"""Tests for crowdsafe.core.potential_field module.
...
7. Empty pedestrians produce zero potential everywhere
...
def single_positive_mass():
    """Single slow pedestrian (m > 0) at origin."""
...
def single_negative_mass():
    """Single fast pedestrian (m < 0) at origin."""
    pos = np.array([[0.0, 0.0]], dtype=np.float64)
...
@pytest.fixture
def symmetric_pair():
    """Two identical positive-mass pedestrians at symmetric positions."""
...
class TestOptimizeTrafficLight:
    ...
    def test_pure_congestion_prefers_balanced_split(self):
        ...
        # Place pedestrians symmetrically in NS and EW corridors
        pos = np.array(
            [
                [0.0, 50.0],  # NS
                [0.0, -50.0],  # NS
...
# ======================================================================
# Test 7: Empty pedestrians -> zero potential
# ======================================================================


class TestEmptyPedestrians:
    """No pedestrians should produce zero potential everywhere."""
    ...
    def test_optimize_with_no_pedestrians(self):
        pos = np.empty((0, 2), dtype=np.float64)
        mass = np.empty((0,), dtype=np.float64)
        intersection = np.array([0.0, 0.0], dtype=np.float64)
...
class TestRMinClamping:
    """Potential should be bounded when evaluation point coincides with pedestrian."""

    def test_coincident_point(self):
        """Evaluation point at pedestrian location uses r_min, not r=0."""
        pos = np.array([[50.0, 50.0]], dtype=np.float64)
        mass = np.array([1.0], dtype=np.float64)
        eval_pts = np.array([[50.0, 50.0]], dtype=np.float64)  # same as pedestrian
...
# ======================================================================
# Test 10: Superposition -- multi-pedestrian potential
# ======================================================================
...
class TestSuperposition:
    ...
    def test_two_pedestrians_superposition(self):
