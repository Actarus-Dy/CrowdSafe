...
from crowdsafe.core.integrator import _clip_speed, adaptive_dt, leapfrog_step
...
class TestSpeedClipping:
    ...
    def test_speed_capped_per_pedestrian_vmax(self):
        n = 4
        positions = np.zeros((n, 2), dtype=np.float64)
        positions[:, 0] = np.arange(n, dtype=np.float64) * 100.0
        velocities = np.zeros((n, 2), dtype=np.float64)
        ...
        for _ in range(50):
            ...
            for i in range(n):
                assert speeds[i] <= v_max_per[i] + 1e-12, (
                    f"Pedestrian {i}: speed {speeds[i]:.4f} exceeds v_max={v_max_per[i]}"
                )
