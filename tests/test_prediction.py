"""Tests for CrowdSimulation prediction (clone + predict).
...
@pytest.fixture()
def sim() -> CrowdSimulation:
    """A small simulation with 20 pedestrians for fast tests."""
    s = CrowdSimulation(G_s=5.0, beta=0.5, adaptive_dt=False, dt=0.1)
    ...
    s.init_pedestrians(positions, velocities, densities)
...
class TestClone:
    def test_clone_has_same_state(self, sim: CrowdSimulation) -> None:
    ...
    def test_clone_is_independent(self, sim: CrowdSimulation) -> None:
    ...
    def test_clone_preserves_obstacles(self, sim: CrowdSimulation) -> None:
...
class TestPredict:
    def test_predict_returns_valid_state(self, sim: CrowdSimulation) -> None:
        ...
        assert result["positions"].shape == (sim.n_pedestrians, 2)
        assert result["n_steps_run"] > 0
        assert result["horizon_s"] >= 10.0

    def test_predict_does_not_mutate_original(self, sim: CrowdSimulation) -> None:
    ...
    def test_predict_diverges_from_current(self, sim: CrowdSimulation) -> None:
    ...
    def test_predict_short_horizon(self, sim: CrowdSimulation) -> None:
        """Very short horizon should run at least 1 step."""
        result = sim.predict(horizon_s=0.01)
        assert result["n_steps_run"] >= 1

    def test_predict_zero_pedestrians(self) -> None:
        """predict() with 0 pedestrians should return immediately, not hang."""
        s = CrowdSimulation(G_s=5.0, adaptive_dt=False, dt=0.1)
        s.init_pedestrians(
...
class TestPredictEndpoint:
    ...
    def test_predict_returns_pedestrians(self, client: TestClient) -> None:
        ...
        assert body["n_pedestrians"] == 15
        assert body["horizon_s"] >= 10.0

        # Each pedestrian has expected keys
        v = body["pedestrians"][0]
