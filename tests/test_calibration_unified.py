...
These tests validate the core scientific claim of the drag-enriched CrowdSafe model:
    a_i = F_gravity / |m_i| + gamma * (v_eq(rho) - |v_i|) * direction

The enrichment is physically motivated (engine vs aerodynamic drag), NOT a
car-following rule.  Gravity provides inter-pedestrian interactions; drag provides
the baseline speed-density relationship.

Three test classes:
    1. TestUnifiedParameterExists -- at least one parameter set satisfies both
    2. TestDefaultCalibration    -- default CrowdSimulation params give R^2 > 0.70
...
from crowdsafe.core.calibration_unified import (
...
class TestDefaultCalibration:
    """Verify that the default CrowdSimulation parameters produce
    ...
    def test_default_r_squared_above_070(self) -> None:
        ...
        # Extract defaults from CrowdSimulation
        sim = CrowdSimulation()
...
class TestDefaultEmergence:
    """Verify that the default CrowdSimulation parameters produce
    measurable upstream deceleration from a slow pedestrian perturbation."""
    ...
    def test_downstream_fluidity_preserved(self) -> None:
        """Downstream pedestrians should not decelerate as much as upstream."""
        sim = CrowdSimulation()
        emg = run_emergence_test(
            G_s=sim.G_s,
            beta=sim._mass_assigner.beta,
        ...
        # (gravitational well from slow pedestrian affects upstream more)
        assert emg["downstream_mean_speed"] >= emg["upstream_mean_speed"] - 3.0, (
            f"Downstream pedestrians decelerated more than upstream: "
...
class TestDragZeroRecoversPureGravity:
    ...
    def test_zero_drag_preserves_speed(self) -> None:
        """With gamma=0, all pedestrians at same speed should stay at that speed."""
        n = 20
...
class TestPhysicalConsistency:
    ...
    def test_drag_sign_convention(self) -> None:
        """Drag accelerates slow pedestrians and decelerates fast pedestrians."""
        gamma = 0.3
        v_eq = 20.0

        # Slow pedestrian: v < v_eq -> positive drag (acceleration)
        assert gamma * (v_eq - 15.0) > 0

        # Fast pedestrian: v > v_eq -> negative drag (deceleration)
