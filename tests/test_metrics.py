"""Tests for crowdsafe.core.metrics -- traffic KPI calculator.
...
class TestComputeThroughput:
    def test_known_crossings(self):
        """3 pedestrians cross gate, 2 do not -> correct count."""
        gate_x = 100.0
...
class TestComputeMeanSpeed:
    def test_known_velocities(self):
        """Pedestrians with speeds 3-4-5 triangle -> known speeds."""
        vels = np.array(
...
class TestComputeDelay:
    def test_at_free_flow_zero_delay(self):
        """All pedestrians at v_free -> delay is 0."""
        v_free = 33.33
    ...
    def test_half_free_flow_positive_delay(self):
        """All pedestrians at half v_free -> positive delay."""
        v_free = 33.33
        v_half = v_free / 2.0
...
class TestComputeStops:
    def test_known_count(self):
        """5 pedestrians below threshold, 10 above -> returns 5."""
        threshold = 2.0
...
class TestComputeLevelOfService:
    ...
    def test_los_levels(self, speed_fraction: float, expected_los: str):
        ...
        # Single pedestrian moving in x direction at the target speed
...
class TestComputeTravelTime:
    def test_known_trajectory(self):
        """Pedestrian moves 10 m/step over 10 steps to cover 100 m corridor."""
        dt = 0.5
        start_x = 20.0
        end_x = 120.0
        # 1 pedestrian, linearly increasing x from 0 to 150
        n_steps = 16
        history = []
    ...
    def test_multiple_pedestrians(self):
        """Two pedestrians with different entry times."""
        dt = 1.0
        start_x = 0.0
        end_x = 50.0
        history = []
        for t in range(20):
            # Pedestrian 0: starts at x=-10, moves +5/step
            # Pedestrian 1: starts at x=-30, moves +5/step
            x0 = -10.0 + t * 5.0
            x1 = -30.0 + t * 5.0
        ...
        tt = compute_travel_time(history, start_x, end_x, dt)
        assert tt is not None
        # Both pedestrians take 10 steps to cross 50 m at 5 m/step -> 10 s
        assert tt == pytest.approx(10.0, rel=1e-12)

    def test_no_completion_returns_none(self):
        """No pedestrian reaches end_x -> returns None."""
        dt = 1.0
        history = [
...
class TestEmptyArrays:
    ...
    def test_travel_time_empty_pedestrians(self):
