...
from crowdsafe.scenarios.rivoli import RivoliCorridor
...
# ======================================================================
# Test 6: Final pedestrian count is reasonable
# ======================================================================


class TestPedestrianCount:
    """Number of final pedestrians is reasonable (not 0, not exploding)."""

    def test_fixed_has_pedestrians(self, corridor):
        result = corridor.run_fixed_timing(duration_s=60.0)
        assert result["final_n_pedestrians"] > 0, (
            "Simulation ended with 0 pedestrians -- all escaped or none injected"
        )

    def test_fixed_not_exploding(self, corridor):
        result = corridor.run_fixed_timing(duration_s=60.0)
        # With injection_rate=0.5/s * 2 directions * 60s + ~82 initial
        # we expect roughly < 200 pedestrians (some exit the corridor)
        assert result["final_n_pedestrians"] < 500, (
            f"Pedestrian count {result['final_n_pedestrians']} seems too high"
        )

    def test_optimized_has_pedestrians(self, corridor):
        result = corridor.run_optimized(duration_s=60.0)
        assert result["final_n_pedestrians"] > 0
...
class TestModeDifference:
    ...
    def test_results_differ(self, corridor):
        ...
        pedestrians_differ = fixed["final_n_pedestrians"] != optimized["final_n_pedestrians"]

        assert speeds_differ or stops_differ or throughput_differs or pedestrians_differ, (
...
class TestReproducibility:
    ...
    def test_different_seed_different_result(self):
        ...
        assert (
            ...
            or r1["final_n_pedestrians"] != r2["final_n_pedestrians"]
        )
