...
**Gravitational crowd flow simulator based on the Janus Cosmological Model.**

CrowdSafe models each pedestrian as a gravitational body with signed mass.
Pedestrians slower than the segment average receive **positive mass** (attract
neighbours, form jams), while faster pedestrians receive **negative mass** (repel
...
curl -X POST http://localhost:8000/api/v1/simulate \
  ...
  -d '{"grid_rows": 5, "grid_cols": 5, "n_pedestrians": 200}'
...
|   +-- simulation.py              CrowdSimulation pipeline (orchestrator)
...
|   +-- pedestrian_agent.py          Passive agent -- physics pushed from simulation
|   +-- intersection_agent.py     Traffic light with red-light mass injection
|   +-- crowd_model.py          Mesa Model wiring simulation + agents
|
...
- Same-sign masses (`m_i * m_j > 0`): attraction -- pedestrians cluster, jams form
- Opposite-sign masses (`m_i * m_j < 0`): repulsion -- pedestrians spread, flow improves
...
Slow pedestrians (below segment average) get positive mass. Fast pedestrians get
...
fundamental diagram). Gravity provides inter-pedestrian interactions and
...
| `rho_jam`   |   150.0 | Jam density (pedestrians/km)          |
...
| GET    | `/api/v1/status`         | Current simulation status (step, pedestrian count)  |
...
| WS     | `/ws/stream/potential`   | Real-time frames at 1 Hz (pedestrians + potential)  |
...
- **Pedestrian map** -- Colour-coded dots: red for positive-mass (slow/congested)
  pedestrians, blue for negative-mass (fast/fluid) pedestrians, positioned on the
  road network grid.
...
- **Live KPI panel** -- Mean speed (km/h), congestion index, pedestrian count,
  and Level of Service (A-F) displayed in real time.
- **Playback controls** -- Start, stop, and single-step buttons. Configuration
  panel to adjust pedestrian count, grid size, and physics parameters without
...
| Backend        | 1,000 pedestrians | 5,000 pedestrians | 10,000 pedestrians |
|----------------|---------------:|---------------:|----------------:|
...
CrowdSafe is validated against the Greenshields fundamental diagram and
...
**Emergence validation:** When a single slow pedestrian is injected into a
...
(CrowdSimulation is not fork-safe).
...
# With coverage
python -m pytest tests/ --cov=crowdsafe --cov-report=term-missing
```
...
CrowdSafe is released under the [MIT License](LICENSE).
